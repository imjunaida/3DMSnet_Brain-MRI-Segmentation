...
for case_idx in range(11, 13) :
    T1_test_vol = sg.read_vol(case_idx, 'T1')[:144, :192, :256]
    T2_test_vol = sg.read_vol(case_idx, 'T2')[:144, :192, :256]
    
    x_test = np.zeros((6916, 2, 27, 27, 27))
    x_test[:, 0, :, :, :] = sg.extract_patches(T1_test_vol, patch_shape=(27, 27, 27), extraction_step=(9, 9, 9))
    x_test[:, 1, :, :, :] =sg. extract_patches(T2_test_vol, patch_shape=(27, 27, 27), extraction_step=(9, 9, 9))
    ...
    segmentation = sg.reconstruct_volume(pred_classes, (144, 192, 256))
    ...
    sg.save_vol(segmentation, case_idx)
